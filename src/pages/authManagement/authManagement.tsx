...
const AuthManagement: React.FC<{ userId: string }> = (props) => {
  ...
  useEffect(() => {
    ...
    myGet('/User/hasResourceFromUser', { id: props.userId }).then((data) => {
      setAdmin(data.map((item: any) => item.id));
    });
  }, [props.userId]);
      ...
      render: (text: any, record: any, index: number) => {
        return (
          <div>
            ...
            <Button type="link" style={{ color: '#989898', cursor: 'auto' }}>
              配置界面
            </Button>
          </div>
        );
      ...
      <div className={st.main}>
        <div style={{ display: 'flex', justifyContent: 'space-between' }}>
          {props.userId == '0' ? (
            ...
            <div className={st.table2} style={{ width: '100%' }}>
              <Table
                rowKey="id"
                key={admin.join()}
                rowSelection={{
        ...
        <p>
          {props.userId == '0'
            ? '注：黄色栏列表的勾选和编辑，功能为开放给游客用户查看；'
            : '注：蓝色列的勾选和编辑，功能为开放给企业管理员用户进行内容编辑、页面预览和发布'}
        </p>
      </div>
    </div>
  );
